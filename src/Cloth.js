...
export default class Cloth {
  ...
  randomize() {
    this.params.map = Boolean(Math.round(Math.random()));
    this.params.seed = Math.random() * 1000;
    this.params.x_multiplier = Math.random() * 45;
    this.params.y_multiplier = Math.random() * 45;
    this.params.polar = Boolean(Math.round(Math.random()))
    this.params.angle_offset = Math.random() * 6 - 3;
  ...
  render(moire) {
    ...
    for (let r = this.params.inner_hole; r < 1; r += 1 / n_polygons) {
      ...
      for (let j = 0; j < 1; j += 1 / n_vertices) {
        ...
        if (moire == "moire") {
          path.add(
            new paper.Point(
              xy[0] + this.params.moire_x,
              xy[1] + this.params.moire_y
            )
          );
        } else {
      ...
      if (this.params.smoothing_sampling == "radius") {
        let factor = this.params.inversed
          ? r.map(0, 1, 1, 0)
          : r.map(0, 1, 0, 1);
    ...
    this.params.path_method == "spiral"
  ...
  position_texture(r, theta) {
    ...
    if (this.params.polar) {
      ...
      yoffset = this.noise3D(
        r / smoothing,
        theta / smoothing,
        this.params.seed + z
      );
      xoffset = this.noise3D(
        r / smoothing,
        theta / smoothing,
        this.params.seed
      );
      ...
      yoffset = this.noise3D(
        ...
        this.params.seed + z
      ); // if z is the same, has no effect on certain axis
      xoffset = this.noise3D(
        x / this.params.smoothing,
        y / this.params.smoothing,
        this.params.seed
  ...
  init_gui() {
    this.gui.add(this.params, "map").listen().onChange((value) => {
      this.params.map = value;
      this.reset();
    });

    this.gui
      .add(this.params, "path_method", ["spiral", "polygons"]).listen()
      .onChange((value) => {
    ...
    noise.add(this.params, "seed", 0, 2000).listen().onChange((value) => {
      this.params.seed = value;
      this.reset();
    });

    noise.add(this.params, "smoothing", 0, 200).listen().onChange((value) => {
      this.params.smoothing = value;
      this.reset();
    });

    noise.add(this.params, "x_multiplier", 0, 100).listen().onChange((value) => {
      this.params.x_multiplier = value;
      this.reset();
    });

    noise.add(this.params, "y_multiplier", 0, 100).listen().onChange((value) => {
      this.params.y_multiplier = value;
      this.reset();
    });

    noise.add(this.params, "polar").listen().onChange((value) => {
      this.params.polar = value;
      this.reset();
    });

    noise.add(this.params, "single_axis_noise").listen().onChange((value) => {
    ...
    moire.add(this.params, "moire").listen().onChange((value) => {
      this.params.moire = value;
      this.reset();
    });

    moire.add(this.params, "moire_x", 0, 10).listen().onChange((value) => {
      this.params.moire_x = value;
      this.reset();
    });

    moire.add(this.params, "moire_y", 0, 10).listen().onChange((value) => {
    ...
    polygons.add(this.params, "inversed").listen().onChange((value) => {
      this.params.inversed = value;
      this.reset();
    });

    polygons
      .add(this.params, "n_polygons", 0, 200)
      .step(1).listen()
      .onChange((value) => {
    ...
    polygons
      .add(this.params, "n_vertices", 3, 50)
      .step(1).listen()
      .onChange((value) => {
    ...
    polygons.add(this.params, "angle_offset", -3, 3).listen().onChange((value) => {
      this.params.angle_offset = value;
      this.reset();
    });

    polygons.add(this.params, "radius", 2, 20).listen().onChange((value) => {
      this.params.radius = value;
      this.reset();
    });

    polygons.add(this.params, "inner_hole", 0, 1).listen().onChange((value) => {
      this.params.inner_hole = value;
      this.reset();
    });

    polygons.add(this.params, "smooth_path").listen().onChange((value) => {
      this.params.smooth_path = value;
      this.reset();
    });

    polygons
      .add(this.params, "smoothing_method", [
        /* 'continuous', 'asymmetric',  */ "catmull-rom",
        "geometric",
      ]).listen()
      .onChange((value) => {
    ...
    polygons
      .add(this.params, "smoothing_sampling", ["radius", "noise"]).listen()
      .onChange((value) => {
    ...
    polygons
      .add(this.params, "path_noise_smoothing", 0, 1).listen()
      .onChange((value) => {
    ...
    style.add(this.params, "strokeWidth", 0, 5).listen().onChange((value) => {
